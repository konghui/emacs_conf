...
;; add lsp-mode to warning-suppress-types and control it not show warnning
(require 'warnings)
...
;; enable pip install python path
(setq exec-path
      (append exec-path '("/Users/larry/Library/Python/2.7/bin")))
;; enable npm install nodejs path
(setq exec-path
      (append exec-path '("/usr/local/bin")))

;; if use env add path to exec-path will cause env can't found node
...
;; dockfile language server can found here:https://github.com/rcjsuen/dockerfile-language-server-nodejs
...
;; use lsp javascript and typescript
